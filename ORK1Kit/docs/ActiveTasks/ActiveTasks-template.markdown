...
The ORK1Kit™ framework includes a number of predefined tasks, which fall into six categories: motor activities, fitness, cognition, voice, audio, and hole peg. The table below summarizes each task and describes the data it generates.

<table>
<caption>Active tasks in ORK1Kit</caption>
<thead>
    <tr>
        <td>Category</td>
        <td>Task</td>
...
`ORK1PredefinedTaskOption` constants when you instantiate one of the predefined
active tasks.
...
data, use `ORK1PredefinedTaskOptionExcludeHeartrate`.


#### Fitness

In the fitness task (see the method [ORK1OrderedTask fitnessCheckTaskWithIdentifier:intendedUseDescription:walkDuration:restDuration:options]([ORK1OrderedTask fitnessCheckTaskWithIdentifier:intendedUseDescription:walkDuration:restDuration:options:])), the user walks for a specified duration (usually
...
All of the data is collected from public <a href="https://developer.apple.com/library/ios/documentation/CoreMotion/Reference/CoreMotion_Reference/index.html">CoreMotion</a> and <a href="https://developer.apple.com/library/ios/documentation/HealthKit/Reference/HealthKit_Framework/">HealthKit</a> APIs on iOS, and serialized to JSON. No analysis is applied to the data by the ORK1Kit framework.

#### Audio 

In the audio task (see the method [ORK1OrderedTask audioTaskWithIdentifier:intendedUseDescription:speechInstruction:shortSpeechInstruction:duration:recordingSettings:options]([ORK1OrderedTask audioTaskWithIdentifier:intendedUseDescription:speechInstruction:shortSpeechInstruction:duration:recordingSettings:options:])), the user makes a sustained sound, and an audio
recording is made. Analysis of the audio data is not included in the
ORK1Kit framework, but might naturally involve looking at the power spectrum
and how it relates to the ability to produce certain
sounds. The ORK1Kit framework uses the <a href="https://developer.apple.com/library/ios/documentation/AVFoundation/Reference/AVFoundationFramework/">AVFoundation framework</a> to collect this
data and to present volume indication during recording. No data
analysis is done by ORK1Kit; you can define your analysis on this
...
In the gait and balance task (see the method [ORK1OrderedTask shortWalkTaskWithIdentifier:intendedUseDescription:numberOfStepsPerLeg:restDuration:options]([ORK1OrderedTask shortWalkTaskWithIdentifier:intendedUseDescription:numberOfStepsPerLeg:restDuration:options:])),
...
In the tapping task (see the method [ORK1OrderedTask twoFingerTappingIntervalTaskWithIdentifier:intendedUseDescription:duration:options]([ORK1OrderedTask twoFingerTappingIntervalTaskWithIdentifier:intendedUseDescription:duration:options:])), the user rapidly alternates between tapping two
...
collected using public APIs. No analysis is performed by the ORK1Kit framework on the data.
...
In the spatial memory task (see the method [ORK1OrderedTask spatialSpanMemoryTaskWithIdentifier:intendedUseDescription:initialSpan:minimumSpan:maximumSpan:playSpeed:maxTests:maxConsecutiveFailures:customTargetImage:customTargetPluralName:requireReversal:options:]([ORK1OrderedTask spatialSpanMemoryTaskWithIdentifier:intendedUseDescription:initialSpan:minimumSpan:maximumSpan:playSpeed:maxTests:maxConsecutiveFailures:customTargetImage:customTargetPluralName:requireReversal:options:])),
...
The Paced Serial Addition Test (PSAT) task (see method [ORK1OrderedTask PSATTaskWithIdentifier:intendedUseDescription:presentationMode:interStimulusInterval:stimulusDuration:seriesLength:options]([ORK1OrderedTask PSATTaskWithIdentifier:intendedUseDescription:presentationMode:interStimulusInterval:stimulusDuration:seriesLength:options:])) measures the cognitive function that assesses auditory and/or visual information processing speed, flexibility, and the calculation ability of the user.
...
The score for the PSAT task is the total number correct answers out of the number of possible correct answers. Data collected by the task is in the form of an `ORK1PSATResult` object. 
...
In the reaction time task (see the method [ORK1OrderedTask reactionTimeTaskWithIdentifier:intendedUseDescription:maximumStimulusInterval:minimumStimulusInterval:thresholdAcceleration:numberOfAttempts:timeout:successSound:timeoutSound:failureSound:option]([ORK1OrderedTask reactionTimeTaskWithIdentifier:intendedUseDescription:maximumStimulusInterval:minimumStimulusInterval:thresholdAcceleration:numberOfAttempts:timeout:successSound:timeoutSound:failureSound:options:])),
...
Data collected by this task is in the form of `ORK1ReactionTimeResult` objects. Each of these objects contain a timestamp representing the delivery of the stimulus and an `ORK1FileResult` object that references the motion data collected during an attempt. To present this task, use an `ORK1TaskViewController` object. 
...
In the tone audiometry task (see the method [ORK1OrderedTask toneAudiometryTaskWithIdentifier:intendedUseDescription:speechInstruction:shortSpeechInstruction:toneDuration:options:]([ORK1OrderedTask toneAudiometryTaskWithIdentifier:intendedUseDescription:speechInstruction:shortSpeechInstruction:toneDuration:options:])),
...
In the Tower of Hanoi task (see the method [ORK1OrderedTask towerOfHanoiTaskWithIdentifier:intendedUseDescription:numberOfDisks:options:]([ORK1OrderedTask towerOfHanoiTaskWithIdentifier:intendedUseDescription:numberOfDisks:options:])), the user is asked to solve the classic Tower of Hanoi puzzle in a minimum number of moves. To solve the puzzle, the user must move the entire stack to the highlighted platform in as few moves as possible. This task measures the user's problem solving skills. A Tower of Hanoi task finishes when the user completes the puzzle correctly or concedes that they cannot solve the puzzle.
 
Data collected by this task is in the form of an `ORK1TowerOfHanoiResult` object. It contains every move taken by the user and indicates whether the puzzle was successfully completed or not.
...
The `ORK1HolePegTestPlaceStep` class represents the place step. In this step, the user uses two fingers to touch the peg and drag it into the hole using their left hand.

The `ORK1HolePegTestRemoveStep` class represents the remove step. Here, the user moves the peg over a line using two fingers on their right hand.
...
`ORK1Result` objects in memory. It is up to you to serialize this
...
`ORK1FileResult` object is included in the hierarchy instead. The file result
references a file in the output directory (specified by the
`outputDirectory` property of `ORK1TaskViewController`). For example,
...
(`ORK1RecorderConfiguration`). A recorder configuration defines a type of
...
The recorders used by ORK1Kit's predefined active tasks always use
...
custom subclasses of `ORK1ActiveStep` and
`ORK1ActiveStepViewController`. In doing this, you can follow the example of
the active steps in the predefined tasks that are already in the ORK1Kit framework. (A helpful tutorial on creating active tasks <a href="http://blog.shazino.com/articles/dev/researchkit-new-active-task/">can be found here</a>).



Some of the steps used in the predefined tasks may also be useful as guides
when you create your own tasks. For example, the `ORK1CountdownStep` displays a timer that counts
down with animation for the step duration. To give another example,
the `ORK1CompletionStep` object displays a confirmation that the task
is completed.
